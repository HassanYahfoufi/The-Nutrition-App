...
class DatabaseHelper
{
  ...
  Future<Database> initializeDatabase() async
  {
    ...
    debugPrint("[DatabaseHelper -> initializeDatabase()] excecuting: Directory directory = await getApplicationDocumentsDirectory();...");
    Directory directory = await getApplicationDocumentsDirectory();
  ...
  */
  ...
  Future<void> createTable(Database db, String tableName, {int tabs = 0}) async
  {
    ...
    if(table[tableName] != null)
    {
      debugPrint("$space[DatabaseHelper -> createTable($tableName)] checkpoint 1");
      table[tableName]!.updateInfo();
      debugPrint("$space[DatabaseHelper -> createTable($tableName)] checkpoint 2");
      String sqlCommand = table[tableName]!.sqlInfo;
      debugPrint("$space[DatabaseHelper -> createTable($tableName)] db.execute($sqlCommand)...");

      await db.execute(sqlCommand);

      //await db.execute("CREATE TABLE $tableType($colID INTEGER PRIMARY KEY AUTOINCREMENT, $colTitle TEXT, $colDescription TEXT, $colPriority INTEGER, $colDate TEXT)");
      debugPrint("$space[DatabaseHelper -> createTable($tableName)] $tableName table created!WWWWWWWWWWWWWWWWWWWWWWWWWWW");
    }
    else
    {
      debugPrint("$space[DatabaseHelper -> createTable($tableName)] $tableName table NOT created!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    }
  ...
  Future<int> insert({required String tableName, required Map<String, dynamic> objectAsMap, int? recursiveIterationCount}) async
  {
    ...
    String tableType = table[tableName]!.type; //String tableType = tableTypes[tableName]!;
    debugPrint("$space[DatabaseHelper -> insert()] After getting tableType");
    //Database? db = getDatabase();
    Database? db = await getDatabase();
    debugPrint("$space[DatabaseHelper -> insert()] After getting the $tableName database");
    int result = 0;

    
    debugPrint("$space[DatabaseHelper -> insert()] Adding to $tableName.....");
    result = await db!.insert(tableType, objectAsMap);//!!!!!!! replace with raw query
    if(result != 0)
    {
      debugPrint("$space[DatabaseHelper -> insert()]  added successfully!!!!!!!!!!!!!!!!!!!");
    }
    else
    {
      debugPrint("$space[DatabaseHelper -> insert()] Failed!  couldn't be added XXXXXXXXXX");
    }
  ...
  Future<List<String>> getTableNames() async
  {
    ...
    List<Map<String, dynamic>> result = await db!.rawQuery("SELECT name FROM sqlite_master WHERE type ='table' AND name NOT LIKE 'sqlite_%'");
    List<String> names = [];
    int index = 0;
    ...
    for(Map<String, dynamic> item in result)
    {
      ...
      {
        debugPrint("\tkey: $key\tvalue: ${item[key]}\ttype:${item[key].runtimeType}");
  ...
  Future<int?> getCount({required String tableName}) async
  {
    ...
    debugPrint("[DatabaseHelper -> getCount($tableName)] excecuting: String tableType = table[tableName]!.type;...");
    if(table.isEmpty)
    {
    ...
    debugPrint("[DatabaseHelper -> getCount($tableName)] String tableType = table[tableName]!.type; COMPLETE");
    Database? db = await getDatabase();
    debugPrint("[DatabaseHelper -> getCount($tableName)] excecuting: List<Map<String, dynamic>> x = await db!.rawQuery(\"SELECT COUNT (*) from $tableType\");...");
    List<Map<String, dynamic>> x = await db!.rawQuery("SELECT COUNT (*) from $tableType");
    debugPrint("[DatabaseHelper -> getCount($tableName)] excecuting: int? result = Sqflite.firstIntValue(x);...");
  ...
  Future<bool> doesExist({required String tableName, required String column, required String value})async
  {
    ...
    debugPrint("[DatabaseHelper -> doesExist($tableName, $column, $value)] End");
    return valueExists;
  ...
  Future<List<Map<String, dynamic>>> getMatchingRows_WhereColumnInValues({required String tableName, required String column, required List<String> values})async
  {
    String tableType = table[tableName]!.type;
    Database? db = await getDatabase();
    ...
    debugPrint("[DatabaseHelper -> getMatchingRowsWhereColumnInValues()] listSQL: $listSQL");
    ...
    listSQL = "$listSQL)";
    debugPrint("[DatabaseHelper -> getMatchingRowsWhereColumnInValues()] listSQL: $listSQL");
    debugPrint("[DatabaseHelper -> getMatchingRowsWhereColumnInValues()] SELECT * FROM $tableType WHERE $column IN ${values.toString()}...");
    List<Map<String, dynamic>> result = await db!.rawQuery("SELECT * FROM $tableType WHERE $column IN $listSQL");
    return result;
  }
  ...
  Future<List<Map<String, dynamic>>> getMatchingColumns({required String tableName, required String column, required String value, required List<String> outputColumns})async
  {
    ...
    Database? db = await getDatabase();
    //List<Map<String, dynamic>> result = await db!.rawQuery("SELECT * FROM $tableType WHERE $column = $value");
    debugPrint('[DatabaseHelper -> getMatchingColumns()] SELECT $outputColumnsSQL FROM $tableType WHERE $column=?", [value]...');
    List<Map<String, dynamic>> result = await db!.rawQuery("SELECT $outputColumnsSQL FROM $tableType WHERE $column=?", [value]);
  ...
  Future<List<Map<String, dynamic>>> getMatchingColumns_WhereColumns({required String tableName, required Map<String,dynamic> conditions, required List<String> outputColumns})async
  {
    ...
    conditions.forEach((key, value) {connditionSQL = "$connditionSQL AND $key = ${value.toString()}";});
    if(connditionSQL.startsWith(" AND "))
    {
      debugPrint("\tconnditionSQL(before): $connditionSQL");
  ...
  Future<List<Map<String, dynamic>>> getMatchingRows_FromM2M({required String tableName, required String column, required List<String> values})async
  {
    ...
    String tableType = table[tableName]!.type;
    Database? db = await getDatabase();
    debugPrint("[DatabaseHelper -> getMatchingRows_FromM2M()] Database retrieved");
    String listSQL;
    listSQL = "(${values.elementAt(0)}";
    debugPrint("[DatabaseHelper -> getMatchingRows_FromM2M()] listSQL: $listSQL");
    for(int i = 1; i < values.length; i++)
    {
      listSQL = "$listSQL, ${values.elementAt(i)}";
    }
    ...
    debugPrint("[DatabaseHelper -> getMatchingRows_FromM2M()] listSQL: $listSQL");
    //List<Map<String, dynamic>> result = await db!.rawQuery("SELECT * FROM $tableType WHERE $column = $value");
    debugPrint("[DatabaseHelper -> getMatchingRows_FromM2M()] SELECT * FROM $tableType WHERE $column IN ${values.toString()}...");
    List<Map<String, dynamic>> result = await db!.rawQuery("SELECT * FROM $tableType WHERE $column IN $listSQL");
  ...
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  Future<List<Map<String, dynamic>>> getRowsFromAccount_WhereColumns({required String tableName, required String column, required int accountID, required String tableName_M2M, required String column_M2M})async
  {
    //get list of ids account_id = widget.account.id
    Map<String, int> condition = <String, int>{};
    condition["account_id"] = accountID;
    debugPrint("[Database Helper -> getRowsFromAccount_WhereColumns()] Getting object ids...");
    List<Map<String, dynamic>> objectIDsFromDatabase = await getMatchingColumns_WhereColumns(tableName: tableName_M2M, conditions: condition, outputColumns: [column_M2M]);
    List<String> objectIDs = objectIDsFromDatabase.map((objectFromDatabase) => objectFromDatabase[column_M2M].toString()).toList();
    debugPrint("$column_M2M: ${objectIDs.toString()}");

    
    debugPrint("[Database Helper -> getRowsFromAccount_WhereColumns()] Getting catagories from database...");
    String tableType = table[tableName]!.type;
    Database? db = await getDatabase();
    ...
    String listSQL;
    listSQL = "(${values.elementAt(0)}";
    debugPrint("[DatabaseHelper -> getRowsFromAccount_WhereColumns()] listSQL: $listSQL");
    for(int i = 1; i < values.length; i++)
    {
      listSQL = "$listSQL, ${values.elementAt(i)}";
    }
    ...
    listSQL = "$listSQL)";
    debugPrint("[DatabaseHelper -> getRowsFromAccount_WhereColumns()] listSQL: $listSQL");
    debugPrint("[DatabaseHelper -> getRowsFromAccount_WhereColumns()] SELECT * FROM $tableType WHERE $column IN ${values.toString()}...");
    List<Map<String, dynamic>> result = await db!.rawQuery("SELECT * FROM $tableType WHERE $column IN $listSQL");
  ...
  Future<List<Map<String, dynamic>>> getColumnsFromAccount_WhereColumns({required String tableName, required Map<String,dynamic> conditions, required List<String> outputColumns})async
  {
    debugPrint("[DatabaseHelper -> getMatchingColumns_WhereColumns()] Start");//!!!!!!!!!!!!!!!!!!!!!!!
    String connditionSQL = "";
    //List<String> conditionsList = conditions.forEach((key, value) {return key;});
  ...
  Future<List<Map<String, dynamic>>>getMapList({required String tableName, String orderByCol = ""}) async
  {
    ...
    var result = await db!.rawQuery("SELECT * FROM $tableType$SqlCommand_orderBy");
  ...
  Future<List<dynamic>> getEntireTable({required String tableName, required Function maptoObject}) async
  {
    //int size = await getCount(tableName: tableName) as int;
    debugPrint("[DatabaseHelper -> getEntireTable(...)] Start\n\tGetting list ...");
    debugPrint("[DatabaseHelper -> getEntireTable(...)] before Database? db = await getDatabase()");
    Database? db = await getDatabase();
    debugPrint("[DatabaseHelper -> getEntireTable(...)] before ...mapList = await getMapList...");
    List<Map<String, dynamic>> mapList = await getMapList(tableName: tableName);// as List<Map<String, dynamic>>;
  ...
  Future<int> update({required String tableName, required dynamic tableRow}) async
  {
    //String tableType = tableTypes[tableName]!;
    String tableType = table[tableName]!.type;
    Database? db = await getDatabase();
    var result = await db!.update(tableType, tableRow.toMap(), where: "$colID = ?", whereArgs: [tableRow.id]);
  ...
  { 
    ...
    debugPrint("[DatabaseHelper -> deleteTable()] calculating present count of databases...");
    final int originalCount = await databaseCount();
    int currentCount;

    debugPrint("[DatabaseHelper -> deleteTable()] getting the Application Documents Directory...");
    Directory directory = await getApplicationDocumentsDirectory();
    ...
    if(!exists)
    {
      debugPrint("[DatabaseHelper -> deleteTable()] The file does not exist in the directory!!!");
    }
    ...
    {*/
      ...
      if(table[tableName] != null)
      {
        ...
        {
          debugPrint("[DatabaseHelper -> deleteTable()] Removing other table/database info");
          table.remove(tableName);
        }
        else
        {
          debugPrint("[DatabaseHelper -> deleteTable()] Delete operation (DROP TABLE IF EXISTS $tableType) was called but failed!!!!!!!!!!");
          debugPrint("db.path: ${db.path}");

          debugPrint("[DatabaseHelper -> deleteTable()] Trying again with another method...");
      ...
      {
        debugPrint("[DatabaseHelper -> deleteTable()] Delete failed! table[$tableName] = null");
